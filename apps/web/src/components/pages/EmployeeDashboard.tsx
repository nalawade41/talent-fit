...
export function EmployeeDashboard() {
  ...
  const employeeData = useMemo(() => {
    return employeesData.find(emp => emp.user.email === user?.email) || employeesData[0];
  }, [user]);

  // Get current project allocations for this employee
  const currentAllocations = useMemo(() => {
    return projectAllocationsData.filter(allocation =>
      allocation.employee_id === employeeData.user_id &&
      (!allocation.end_date || new Date(allocation.end_date) > new Date())
    );
  }, [employeeData]);

  // Get project history (past allocations)
  const projectHistory = useMemo(() => {
    return projectAllocationsData.filter(allocation =>
      allocation.employee_id === employeeData.user_id &&
      allocation.end_date &&
      new Date(allocation.end_date) <= new Date()
